...
for file_list = 1:n_files
    ...
    stream = load (fullfile ([directory '/data'], ['primary_sink_coordinates_', output_name, '.mat']));
    stream = stream.s;
    ...
    for kk = 1:nt-1
        ...
        x = data_ext(idx_max_ext).Centroid(1,1);
        y = data_ext(idx_max_ext).Centroid(1,2);
        ...
        plot(stream(kk,1), stream(kk,2), 'mo', 'markersize', 20, 'markerfacecolor', 'm')
        p1 = [track(kk,1) track(kk,2)];
        p3 = [stream(kk,1) stream(kk,2)];
        dpA = p3-p1;
        
        quiver(p1(1),p1(2),dpA(1),dpA(2), 'm', ...
            'linewidth', 1, 'autoscale', 'off', 'maxheadsize', 0.3)
        
        quiver(track(kk,1), track(kk,2), 100*track_diff_unit(kk,1), 100*track_diff_unit(kk,2), 'w',...
            'linewidth', 2, 'autoscale', 'off', 'maxheadsize', 0.5)
        
        set(gcf, 'InvertHardCopy', 'off');
        print(gcf, '-dtiffn', '-r200', [directory '/images/HR/both_vect_', output_name, '_frame' num2str(kk) '_HR.tif']);
